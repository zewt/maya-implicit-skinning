...
void Animated_mesh_ctrl::get_base_potential(std::vector<float> &pot) const  { assert(_animesh != NULL); _animesh->get_base_potential(pot); }
void Animated_mesh_ctrl::set_base_potential(const std::vector<float> &pot) { assert(_animesh != NULL); _animesh->set_base_potential(pot); }
