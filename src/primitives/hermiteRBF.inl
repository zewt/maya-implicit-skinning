...
float HermiteRBF::f_global(const Point_cu& x) const
{
    float ret = 0;
    ...
    for(int i = 0; i < size_off.y; i++)
    {
        Vec3_cu beta;
        Point_cu  pt;
        float alpha = HRBF_env::fetch_weights_point(beta, pt, i + size_off.x);
        Vec3_cu diff =  x - pt;
        float l = diff.norm();
        ...
        // Grad( f o g )(x) = (f' o g)(x).Grad(g)(x)
        if( l > 0.00001f)
            ret += alpha * RBFWrapper::PHI_TYPE::f(l) + beta.dot(diff)*RBFWrapper::PHI_TYPE::df(l)/l;
        #endif
...
IF_CUDA_DEVICE_HOST inline
float HermiteRBF::f(const Point_cu& x) const
{
    const float ret = f_global(x);
...
IF_CUDA_DEVICE_HOST inline
Vec3_cu HermiteRBF::gf_global(const Point_cu& x) const
{
    Vec3_cu grad = Vec3_cu(0., 0., 0.);

    int2 size_off = HRBF_env::fetch_inst_size_and_offset(_id);

    if(size_off.y == 0) return grad;
...
IF_CUDA_DEVICE_HOST inline
Vec3_cu HermiteRBF::gf(const Point_cu& x) const
{
    Vec3_cu grad;
