 ...
 */
#include "common/gizmo2/gizmo_rot2.hpp"
...
void Gizmo_rot2::draw_arc_circle(Axis_t axis,
                                const Vec3_cu& a,
...
void Gizmo_rot2::draw_circle(Axis_t axis,
                            const Vec3_cu& cam_dir,
                            float scale,
                            const Color& cl)
{
...
void Gizmo_rot2::draw(const Camera& cam)
{
    if(!_show) return;
...
void Gizmo_rot2::draw_tangent(const Vec3_cu& cam_dir, float dist_cam)
{
...
bool Gizmo_rot2::select_constraint(const Camera& cam, int px, int py)
{
    if(!_show){
        _selected_axis = NOAXIS;
        return false;
...
TRS Gizmo_rot2::slide(const Camera& cam, int px, int py)
{
    py = cam.height() - py; /////////// Invert because of opengl
...
TRS Gizmo_rot2::axis_rotation(const Camera& cam, int px, int py)
{
    // Start pixel is registered not inverted we need to invert it
    Vec3_cu picked = Vec3_cu((float)_start_pix.x, (float)(cam.height() - _start_pix.y), 0.f);
    ...
    // Rotation in global coordinates
    return TRS::rotation(_axis, angle);
}
