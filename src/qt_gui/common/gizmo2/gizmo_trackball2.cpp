 ...
 */
#include "common/gizmo2/gizmo_trackball2.hpp"
...
Gizmo_trackball2::Gizmo_trackball2() : Gizmo2(),
    _rad_sphere(6.5f),
    _rad_handles(1.0f),
    _ortho_factor(20.f),
...
void Gizmo_trackball2::draw(const Camera& cam)
{
    if(!_show) return;
...
bool Gizmo_trackball2::select_constraint(const Camera& cam, int px, int py)
{
    if(!_show){
        _selected_axis = NOAXIS;
        return false;
...
TRS Gizmo_trackball2::slide(const Camera& cam, int px, int py)
{
    py = cam.height() - py; /////////// Invert because of opengl
...
TRS Gizmo_trackball2::trackball_rotation(const Camera& cam, int px, int py)
{
    const Point_cu org    = Convs::to_point(_start_frame.get_translation());
    const Vec3_cu  picked = Vec3_cu((float)_start_pix.x, cam.height() - (float)_start_pix.y, 0.f); // Has been registerd inverted
    ...
    return TRS::rotation( axis, angle);
#else
    ...
    // Rotation in world coordinates
    return TRS::rotation(world_axis, angle);
#endif
